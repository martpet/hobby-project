...
export async function resolveEncryptionPassword(): Promise<string> {
  const fromEnv = getEnv("BACKUP_ENCRYPTION_PASSWORD");

  if (fromEnv) {
    return validate(fromEnv, "BACKUP_ENCRYPTION_PASSWORD");
  }

  const service = getEnv("BACKUP_KEYCHAIN_SERVICE") ?? "hobproj-backup";

  if (service === "") {
    throw new Error(
      "Missing BACKUP_ENCRYPTION_PASSWORD and the Keychain lookup is disabled.",
    );
  }
  ...
  if (code !== 0 || stdout === "") {
    throw new Error(
      `Could not read the backup password from the Keychain item '${service}'. ` +
        ...
        "or set BACKUP_ENCRYPTION_PASSWORD in the environment.",
    );
  }
