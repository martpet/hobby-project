...
// passkey-actions.js) only bind DOM events to these functions.
...
// Confirms with the user before a silent step-up reauth (see `withReauth`):
...
function confirmReauth() {
  return new Promise((resolve) => {
    ...
    dialog.innerHTML = `
      ...
      <p>You'll be asked to use a passkey again before continuing.</p>
      <footer class="actions">
        <button class="cancel">Cancel</button>
        <button class="continue" autofocus>Continue</button>
      </footer>
    `;
    ...
    dialog.addEventListener("close", () => {
      ...
      dialog.remove();
      resolve({ confirmed: false, dialog: null });
    });
...
// Runs `run`, and on a REAUTH_REQUIRED result confirms with the user (see
// confirmReauth) and reauthenticates with a passkey (which replaces the
// session, same cookie name) and retries `run` once — the user only sees
// the confirm dialog then the passkey prompt. A failed reauth is returned
// as the result; a declined confirm or cancelled ceremony rejects.
async function withReauth(run) {
  ...
  if (result.code === "REAUTH_REQUIRED") {
    const { confirmed, dialog } = await confirmReauth();
